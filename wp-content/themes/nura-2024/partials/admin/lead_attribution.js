// version = '1.0.1'; // IMPORTANT: Make a note in CHANGELOG.md!
...
jQuery(document).ready(function($) { 
	...
	// ======================================= //
	//          FIND ELEMENT BY STRING         //
	// ======================================= // 

	/**
	 * Find elements whose id OR class names contain a given substring
	 * @param {string} str - The substring to search for
	 * @returns {jQuery} - A jQuery collection of matched elements
	 */
	function findFieldByName(str) {
	    var lowerStr = str.toLowerCase();
	    return $('*').filter(function() {
	        var id  = (this.id || '').toLowerCase();
	        var cls = ($(this).attr('class') || '').toLowerCase();
	        return (id.indexOf(lowerStr) !== -1 || cls.indexOf(lowerStr) !== -1);
	    });
	}
	...
	function populatefields(targetblock, campaignfield, sourcefield, mediumfield, attrsrcfield, visitsfield) {
		...
		if (targetblock.hasClass('cognito')) {
			...
			var campaignfield = findFieldByName(campaignfield);
			var sourcefield = findFieldByName(sourcefield);
			var mediumfield = findFieldByName(mediumfield);
			var attrsrcfield = findFieldByName(attrsrcfield);
			var visitsfield = findFieldByName(visitsfield);

			campaignfield.val(campaign);
			sourcefield.val(source);
			mediumfield.val(medium);
			attrsrcfield.val(attrsrc);
			visitsfield.val(visits);
