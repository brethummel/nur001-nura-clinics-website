<?php
/*
Template Name: Abstracts & Posters
Partial Name: singlet_post-abstracts
*/
?>
<?php $result = $args; 
	...
	$settings = get_field('post_settings', $result->ID);
	// echo('<pre>'); print_r($settings); echo('</pre>');
	$attribution = get_field('post_attribution', $result->ID);
	// echo('<pre>'); print_r($attribution); echo('</pre>');

	if ($settings['type'] == 'pdf') {
		$link = $settings['pdf']['url'];
		$target = '_blank';
...
<!-- BEGIN ABSTRACTS SINGLET -->
<div class="post-container<?php echo($classes); ?><?php if ($filters) { echo(' ' . $mycat); }?> just-loaded"<?php if ($filters) { ?> data-cats="<?php echo($cats); ?>" <?php } ?> data-page="<?php echo($pagenum); ?>">
	<div class="post col-12">
		<a href="<?php echo($link); ?>"<?php if (isset($target)) { ?> target="<?php echo($target); ?>"<?php } ?>></a>
		<?php if (isset($myimage) && $myimage && $columns != 1) { ?>
		...
		<div class="text-container">
			<?php if ($attribution['date']) { ?><p class="date"><?php echo($attribution['date']); ?></p><?php } ?>
			...
			<?php if ($mytitle) { ?>
				<?php 
					...
					$mytitle = spr_highlight_matches($mytitle, $searchterms)[1];
					$match = spr_highlight_matches($mytitle, $searchterms)[0];
				?>
				<h4><a href="<?php echo($link); ?>"<?php if (isset($target)) { ?> target="<?php echo($target); ?>"<?php } ?>><?php echo($mytitle); ?></a><?php if (isset($_GET['sc']) && !$match) { ?><span class="content-match">content&nbsp;match</span><?php } ?></h4>
			<?php } ?>
			<?php if (isset($attribution['name']) && $attribution['name'] != '') { ?><p class="attribution">by <?php echo(spr_highlight_matches($attribution['name'], $searchterms)[1]); ?><?php if (isset($attribution['title']) && $attribution['title'] != '') { ?>, <?php echo(spr_highlight_matches($attribution['title'], $searchterms)[1]); ?><?php } ?><?php if (isset($attribution['org']) && $attribution['org'] != '') { ?> <?php echo(spr_highlight_matches($attribution['org'], $searchterms)[1]); ?><?php } ?></p><?php } ?>
