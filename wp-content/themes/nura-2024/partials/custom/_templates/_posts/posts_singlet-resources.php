...
Partial Name: singlet_post-resources
*/
?>
<?php $result = $args; 
	...
	$settings = get_field('resources_settings', $result->ID);
...
<div class="post-container<?php echo($classes); ?><?php if ($filters) { echo(' ' . $mycat); }?><?php if ($pagenum > 1) { ?> just-loaded<?php } ?>"<?php if ($filters) { ?> data-cats="<?php echo($cats); ?>" <?php } ?> data-page="<?php echo($pagenum); ?>">
	<div class="post col-12">
		...
		<div class="video-container">
			<?php if ($settings['type'] == 'video') { ?>
				<?php echo($settings['video']['video']); ?>
			<?php } ?>
			<?php if ($settings['type'] == 'iframe') { ?>
				<iframe loading="lazy" src="<?php echo($settings['iframe']['source']); ?>" style="aspect-ratio:16/9;"<?php if (isset($imagecol['iframe']['allow'])) { ?> allow="<?php echo($settings['iframe']['allow']); ?>"<?php } ?> allowfullscreen frameborder="0"></iframe>
			<?php } ?>
		</div>
	</div>
</div>
