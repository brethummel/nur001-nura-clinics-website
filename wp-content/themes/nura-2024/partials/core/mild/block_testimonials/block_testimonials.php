...
<?php $block = $args; 
	...
	/** ===== WORD COUNT MEDIAN =====
	 * Get median word count for testimonials quotes.
	 * Strips HTML, normalizes whitespace, ignores empty quotes.
	 *
	 * @param array $quotes
	...
	$medianwords = spr_testimonials_median_wordcount($quotes);
	$shortcutoff = (int)floor($medianwords * 0.6666667);
	$longcutoff  = (int)ceil($medianwords * 1.333333);
			...
			<div class="col-12 slides-container">
				<div class="slides">
					...
					<?php
						...
						if ($medianwords > 0) {
							$wc = spr_testimonial_wordcount($quote['quote'] ?? '');
							if ($wc > 0 && $wc < $shortcutoff) {
								$slideclass .= ' short';
							} elseif ($wc > $longcutoff) {
								$slideclass .= ' long';
							}
						}
					?>
